...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    int page = offsetX / CGRectGetWidth(_listView.frame);
    if (page == 0) {
        ...
        if (page != offsetX / CGRectGetWidth(_listView.frame)) {
            // 一旦出现偏移，取整修正 ContentOffset（page为int类型，实际为向下取整）
            [_listView.scrollView setContentOffset:CGPointMake(page * CGRectGetWidth(_listView.frame), 0) animated:NO];
        }
    }
